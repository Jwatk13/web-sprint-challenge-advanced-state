// ❗ You don't need to add extra action types to achieve MVP
export const MOVE_CLOCKWISE = 'MOVE_CLOCKWISE' //Completed
export const MOVE_COUNTERCLOCKWISE = 'MOVE_COUNTERCLOCKWISE' //Completed
export const SET_QUIZ_INTO_STATE = 'SET_QUIZ_INTO_STATE' //Completed
export const SET_SELECTED_ANSWER = 'SET_SELECTED_ANSWER' //Completed
export const SET_INFO_MESSAGE = 'SET_INFO_MESSAGE' // Working
export const INPUT_CHANGE = 'INPUT_CHANGE' // Untouched
export const RESET_FORM = 'RESET_FORM' // Untouched
